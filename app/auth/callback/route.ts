...
export async function GET(request: Request) {
  ...
  if (code) {
    ...
    if (user && !user.user_metadata?.terms_accepted_at) {
      // Keep the same response so the Supabase session cookies written by
      // exchangeCodeForSession are preserved when sending the user to terms.
      response.headers.set(
        "Location",
        new URL(`/terms?accept=1&next=${encodeURIComponent(next)}`, requestUrl.origin).toString()
      );
      return response;
    }
  }
