...
export default function PlaystationProductActions({product,disabled}:{product:any;disabled:boolean}){
 ...
 function add(){
   ...
   addItem({id:`ps-${product.id}`,name:product.name,price:Number(product.price),image:product.image||undefined,details:[product.model,storage,product.condition].filter(Boolean).join(' • '),options:selectedOptions,note:note.trim()});
 }
