...
export default function CheckoutPage() {
  ...
  const [invoiceTotals, setInvoiceTotals] = useState({subtotal:0,shipping:0,discount:0,total:0});
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!termsAccepted) { alert("وافق على الشروط والأحكام قبل إتمام الطلب."); return; }
    ...
    setInvoiceTotals({subtotal:totalPrice,shipping:shippingCost,discount,total:grandTotal});
      ...
      <>
        ...
        <main className="invoice-page" dir="rtl">
          <section className="invoice-card">
            <div className="invoice-top"><div><span>NOVATEK • INVOICE</span><h1>تم استلام طلبك ✓</h1><p>شكراً لطلبك. هذه فاتورة طلبك وتفاصيله.</p></div><b>{orderNumber}</b></div>
            <div className="invoice-customer"><div><span>العميل</span><b>{form.name}</b></div><div><span>الهاتف</span><b>{form.phone}</b></div><div><span>التوصيل</span><b>{form.governorate} · {form.city}</b></div></div>
            <div className="invoice-items">{invoiceItems.map((item:any)=><div key={item.id} className="invoice-item"><div className="invoice-item-img">{item.image?<img src={item.image} alt=""/>:<span>BK</span>}</div><div><b>{item.name}</b><small>{item.details||""}</small>{item.options?.map((x:string,i:number)=><small key={i}>{x}</small>)}{item.note&&<small>ملاحظة: {item.note}</small>}</div><strong>${(Number(item.price)*Number(item.qty)).toFixed(2)}</strong></div>)}</div>
            <div className="invoice-total"><span>المجموع الفرعي</span><b>${invoiceTotals.subtotal.toFixed(2)}</b><span>الشحن</span><b>${invoiceTotals.shipping.toFixed(2)}</b>{invoiceTotals.discount>0&&<><span>الخصم</span><b>-${invoiceTotals.discount.toFixed(2)}</b></>}<strong>الإجمالي</strong><strong>${invoiceTotals.total.toFixed(2)}</strong></div>
            {paymentMethod === "wallet" && <p className="invoice-paid">💳 تم الدفع من رصيد المحفظة.</p>}
            <div className="invoice-actions"><button onClick={()=>window.print()}>🖨️ طباعة الفاتورة</button><Link href="/track">تتبع الطلب</Link></div>
          </section>
        </main>
        <Footer />
      </>
  ...
  return (
    <>
      ...
      <main className="mx-auto max-w-6xl px-4 py-8 sm:px-6 sm:py-12" dir="rtl">
        <div className="mb-6"><p className="text-xs font-extrabold tracking-widest text-blue">CHECKOUT / NOVATEK</p><h1 className="mt-2 text-3xl font-extrabold text-ink">إتمام الطلب</h1><p className="mt-2 text-sm text-ink-soft">راجع بيانات التوصيل والدفع قبل تأكيد الطلب.</p></div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4 rounded-3xl border border-line bg-white p-5 shadow-sm sm:p-7">
          ...
          <label className="flex items-start gap-3 rounded-2xl border border-line bg-surface p-4 text-xs leading-6 text-ink-soft">
            <input type="checkbox" checked={termsAccepted} onChange={e => setTermsAccepted(e.target.checked)} className="mt-1 h-4 w-4 accent-[var(--orange)]" />
            <span>أوافق على <Link href="/terms" className="font-bold text-orange underline">الشروط والأحكام</Link> وأؤكد أن بيانات الطلب صحيحة.</span>
          </label>
          <button disabled={submitting || !termsAccepted} className="w-full rounded-full bg-orange px-6 py-3.5 text-sm font-extrabold text-white shadow-lg shadow-orange/15 disabled:opacity-60">{submitting ? "جاري الإرسال..." : "تأكيد الطلب"}</button>
        </form>
        <aside className="rounded-3xl border border-line bg-surface p-5 shadow-sm sm:p-6 lg:sticky lg:top-6"><div className="flex items-center justify-between"><h2 className="text-lg font-extrabold text-ink">ملخص الطلب</h2><Link href="/cart" className="text-xs font-bold text-blue">تعديل السلة</Link></div><div className="mt-4 space-y-2">{items.map(item=><div key={item.id} className="rounded-2xl bg-white p-3 text-xs"><div className="flex justify-between gap-3"><span className="min-w-0 font-semibold">{item.name} × {item.qty}</span><b>${(Number(item.price)*Number(item.qty)).toFixed(2)}</b></div>{item.details&&<p className="mt-1 text-[10px] text-ink-soft">{item.details}</p>}{item.options?.map((x:string,i:number)=><p key={i} className="mt-1 text-[10px] text-blue">{x}</p>)}{item.note&&<p className="mt-1 text-[10px] text-orange">ملاحظة: {item.note}</p>}</div>)}</div><div className="mt-4 space-y-2 border-t border-line pt-4 text-sm"><div className="flex justify-between text-ink-soft"><span>المجموع</span><b>${totalPrice.toFixed(2)}</b></div><div className="flex justify-between text-ink-soft"><span>الشحن</span><b>${shippingCost.toFixed(2)}</b></div>{discount>0&&<div className="flex justify-between text-blue"><span>الخصم</span><b>-${discount.toFixed(2)}</b></div>}<div className="flex justify-between border-t border-line pt-3 text-base font-extrabold"><span>الإجمالي</span><span>${grandTotal.toFixed(2)}</span></div></div></aside>
        </div>
      </main>
      <Footer />
    </>
