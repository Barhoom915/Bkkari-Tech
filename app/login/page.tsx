...
function LoginForm() {
  ...
  const [termsOpen, setTermsOpen] = useState(false); const [termsAccepted, setTermsAccepted] = useState(false); const [termsText, setTermsText] = useState("باستخدام NOVATEK، يوافق المستخدم على تقديم معلومات صحيحة عند إنشاء الحساب أو الطلب، وعلى مراجعة تفاصيل المنتج والسعر قبل تأكيد الطلب.");
  ...
  return <main className="login-page" dir="rtl"><div className="login-orb login-orb-one"/><div className="login-orb login-orb-two"/><section className="login-shell">
    <div className="login-showcase"><div className="showcase-grid"/><div className="showcase-top"><a href="/" className="brand-mark"><span>NOVATEK <b>TECH</b></span></a><span className="secure-pill"><span/> دخول آمن</span></div><div className="showcase-content"><span className="eyebrow">{mode === "signup" ? "أهلاً في عائلة NOVATEK" : "مرحباً بك من جديد"}</span><h1>كل عالمك التقني<br/><strong>بمكان واحد.</strong></h1><p>حساب واحد للوصول إلى طلباتك، محفظتك، خدماتك الرقمية ومشاريع المواقع.</p><div className="benefits"><div><i>✓</i><span>تابع طلباتك وحالتها</span></div><div><i>✓</i><span>أدر محفظتك وخدماتك</span></div><div><i>✓</i><span>تواصل معنا بسهولة</span></div></div></div><div className="showcase-footer"><span>لابتوبات</span><span>خدمات رقمية</span><span>برمجة مواقع</span></div></div>
    <div className="login-card-wrap"><div className="login-card"><div className="card-heading"><span className="card-kicker">NOVATEK</span><h2>{mode === "signup" ? "إنشاء حساب جديد" : "تسجيل الدخول"}</h2><p>{mode === "signup" ? "أنشئ حسابك وابدأ باستخدام المتجر" : "ادخل إلى حسابك بالطريقة المناسبة إلك"}</p></div>
      ...
      <form onSubmit={handleEmailAuth} className="email-auth-form">{mode === "signup" && <input value={name} onChange={e=>setName(e.target.value)} placeholder="الاسم الكامل" required/>}<input type="email" value={email} onChange={e=>setEmail(e.target.value)} placeholder="البريد الإلكتروني" required/><input type="password" minLength={6} value={password} onChange={e=>setPassword(e.target.value)} placeholder="كلمة المرور" required/>{mode === "signup" && <label className="signup-terms-check"><input type="checkbox" checked={termsAccepted} onChange={e=>setTermsAccepted(e.target.checked)} /><span>أوافق على <button type="button" onClick={()=>setTermsOpen(true)}>الشروط والأحكام</button></span></label>}<button className="primary-auth" disabled={loading || (mode === "signup" && !termsAccepted)}>{loading ? "جاري التنفيذ..." : mode === "signup" ? "إنشاء الحساب" : "تسجيل الدخول"}</button></form>
      ...
      {mode === "signup" && <button type="button" onClick={()=>setTermsOpen(true)} className="terms-inline">📄 قراءة الشروط والأحكام {termsAccepted ? "✓ تمت الموافقة" : ""}</button>}
