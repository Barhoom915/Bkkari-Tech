...
export default function ProductActions({ laptop }: { laptop: Laptop }) {
  ...
  function handleAdd() {
    addItem({ id: `laptop-${laptop.id}-${Date.now()}`, name: laptop.name, price: laptop.price, image: laptop.images?.[0], details: [laptop.brand,laptop.model,laptop.cpu,laptop.ram,laptop.storage].filter(Boolean).join(" • "), note: note.trim() }, 1);
    setAdded(true);
