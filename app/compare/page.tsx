    ...
    {laptops.length ? <>
      ...
      <section className="compare-detailed-grid">
        {laptops.map(l => <article key={l.id} className={`compare-detailed-card ${bestPerformance?.id === l.id ? "is-best" : ""}`}>
          <div className="compare-detailed-top"><div className="compare-detailed-image">{l.images?.[0] ? <img src={l.images[0]} alt={l.name} /> : <span>BK</span>}</div><div><span className="compare-brand">{l.brand || "NOVATEK"}</span><h2>{l.name}</h2><strong className="compare-big-price">${Number(l.price).toFixed(0)}</strong></div></div>
