...
export async function POST(request: Request) {
  ...
  for (const adminId of adminIds) {
    ...
    for (const row of adminSubscriptions ?? []) {
      try { await webpush.sendNotification(row.subscription as webpush.PushSubscription, JSON.stringify({ title: "🔔 طلب جديد في NOVATEK", body: `وصل طلب جديد ${orderNumber}. افتح لوحة التحكم لمراجعة التفاصيل.`, href: "/orders", icon: "/brand/logo.png" })); }
      catch (err: any) { if (err?.statusCode === 404 || err?.statusCode === 410) await service.from("push_subscriptions").delete().eq("id", row.id); }
    }
  }
