...
async function getDigitalHomeProducts() {
  try {
    ...
    for (const p of available) { if (picked.length >= 24) break; if (!picked.some(x => String(x.id) === String(p.id))) picked.push(p); }
    return picked.slice(0, 24).map(p => ({
      id: Number(p.id), name: p.name, image: p.thumbnail, maxQuantity: p.max_quantity, categories: p.categories ?? [],
...
const defaultPromos = [
  ...
  { title: "تصميم وبرمجة المواقع", text: "متجرك وموقعك من NOVATEK", href: "/web-dev", image: "/catalog/promo-web.png", key: "web" },
  { title: "كمبيوتر مكتبي", text: "اطلب تجميعة حسب استخدامك", href: "/pc-builder", image: "/catalog/promo-desktop.png", key: "desktop" },
];

const defaultMedia = {
  hero: ["/catalog/hero-laptop.png", "/catalog/hero-playstation.png", "/catalog/hero-digital.png", "/catalog/hero-web.png"],
...
export default async function Home() {
  ...
  return <div className="pl-store">
    ...
    <main>
      ...
      {digitalHomeProducts.length > 0 && <DigitalHomeSection products={digitalHomeProducts} banner={mergedMedia.promos.digital || '/catalog/promo-digital.png'} />}
      ...
      <section className="pl-promo-grid">
        {promoTiles.map((tile, i) => <Link href={tile.href} key={tile.href} className={`pl-promo-card p${i}`} style={{ backgroundImage: `url(${tile.image})` }}><div><small>NOVATEK</small><h3>{tile.title}</h3><p>{tile.text}</p><b>تصفح القسم ←</b></div></Link>)}
      </section>
