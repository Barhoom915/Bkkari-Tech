...
function ProductCard({ product, chatMarkupPercent }: { product: SatofillProduct; chatMarkupPercent: number }) {
  ...
  return (
    <Link
      href={`/services/product/${product.id}`} prefetch
...
export default async function ServiceCategoryPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  // SatoFill may return products on the category itself and/or inside nested
  // subcategories. Flatten the whole tree so the store never silently hides
  // products just because they belong to a nested category.
  const flattenProducts = (item: SatofillCategory): SatofillProduct[] => [
    ...(item.products ?? []),
    ...(item.subcategories ?? []).flatMap(flattenProducts),
  ];
