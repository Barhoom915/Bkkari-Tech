...
function ProductCard({ laptop }: { laptop: Laptop }) {
  return <article className="pl-product-card">
    <div className="pl-product-media">
      <Link href={`/laptops/${laptop.id}`} prefetch className="pl-product-image-link">
      ...
      <span className="pl-brand-tag">{laptop.brand || "NOVATEK"}</span>
    ...
    <div className="pl-product-body">
      <Link href={`/laptops/${laptop.id}`} prefetch><h3>{laptop.name}</h3></Link>
      ...
      <Link href={`/laptops/${laptop.id}`} prefetch className="pl-add">إضافة إلى السلة</Link>
    </div>
  </article>;
}
...
function PlaystationCard({ product }: { product: PlaystationProduct }) {
  ...
  return <article className="pl-product-card ps-product-card">
    <div className="pl-product-media">
      <Link href={`/playstation/${product.id}`} prefetch className="pl-product-image-link">
    ...
    <div className="pl-product-body">
      <Link href={`/playstation/${product.id}`} prefetch><h3>{product.name}</h3></Link>
      ...
      <Link href={`/playstation/${product.id}`} prefetch className="pl-add">{out ? "عرض التفاصيل" : "اختيار المنتج ←"}</Link>
    </div>
  </article>;
}
...
export type DigitalHomeProduct = { id: number; name: string; price: number; image?: string | null; maxQuantity?: number; currency_symbol?: string; categories?: string[] };

export function DigitalHomeProductRail({ products }: { products: DigitalHomeProduct[] }) {
  const { addItem } = useCart();
  return <LoopRail
    items={products.slice(0, 12)}
    className="digital-home-rail"
    render={(p) => <article className="pl-product-card digital-home-card">
      <div className="pl-product-media"><Link href={`/services/product/${p.id}`} prefetch className="pl-product-image-link">{p.image ? <img src={p.image} alt={p.name} loading="lazy" decoding="async" /> : <div className="ps-product-placeholder"><b>⚡</b></div>}</Link><span className="pl-brand-tag">DIGITAL</span></div>
      <div className="pl-product-body"><Link href={`/services/product/${p.id}`} prefetch><h3>{p.name}</h3></Link><p>خدمة رقمية</p><div className="pl-price"><strong>${Number(p.price).toFixed(2)}</strong></div><button type="button" className="pl-add" onClick={() => addItem({id:`digital-${p.id}`,name:p.name,price:Number(p.price)})}>إضافة للسلة</button></div>
    </article>}
  />;
}
...
const heroSlides = [
  { image: "/catalog/hero-laptop.png", eyebrow: "NOVATEK", title: "اللابتوبات", text: "لابتوبات للأعمال وGaming وأجهزة مناسبة لكل استخدام", href: "/laptops", button: "تصفح اللابتوبات" },
  ...
  { image: "/catalog/hero-web.png", eyebrow: "NOVATEK WEB", title: "تصميم وبرمجة المواقع", text: "مواقع ومتاجر إلكترونية مصممة حسب حاجتك", href: "/web-dev", button: "اطلب موقعك" },
