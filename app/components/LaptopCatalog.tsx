...
export default function LaptopCatalog({ laptops, initialBrand = "all", initialCategory = "all" }: { laptops: Laptop[]; initialBrand?: string; initialCategory?: string }) {
  ...
  const filtered = useMemo(() => laptops.filter(l => Boolean(l.images?.[0]) && Number(l.price) <= maxPrice), [laptops, maxPrice]);
  ...
  return <div>
    ...
    {filtered.length === 0 ? <div className="mt-6 rounded-3xl border border-dashed border-line bg-surface p-12 text-center text-sm text-ink-soft">ما لقينا أجهزة ضمن السعر المحدد.</div> : <div className="laptop-products-grid mt-6 grid grid-cols-2 gap-2.5 sm:gap-5 lg:grid-cols-3">{filtered.map(l => <article key={l.id} className="group relative flex flex-col overflow-hidden rounded-3xl border border-line bg-white shadow-sm transition duration-300 hover:-translate-y-1 hover:shadow-xl">
      <div className="relative flex aspect-[4/3] items-center justify-center overflow-hidden bg-surface"><Link href={`/laptops/${l.id}`} prefetch className="absolute inset-0">{l.images?.[0] ? <img src={l.images[0]} alt={l.name} className="h-full w-full object-contain p-3 transition duration-500 group-hover:scale-105" loading="lazy" decoding="async"/> : <span className="grid h-full place-items-center p-6"><LaptopArtwork brand={l.brand} category={l.category} /></span>}</Link><div className="absolute right-3 top-3 flex gap-1.5 pointer-events-none">{l.is_new && <span className="rounded-full bg-blue px-2.5 py-1 text-[10px] font-bold text-white">جديد</span>}{l.is_bestseller && <span className="rounded-full bg-orange px-2.5 py-1 text-[10px] font-bold text-white">الأكثر مبيعاً</span>}</div><FavoriteButton laptopId={l.id} className="absolute left-3 top-3 z-10"/></div>
      <div className="flex flex-1 flex-col p-5"><div className="text-xs font-semibold text-blue">{l.brand || "NOVATEK"}</div><Link href={`/laptops/${l.id}`} prefetch><h3 className="mt-1 line-clamp-2 font-bold text-ink hover:text-blue">{l.name}</h3></Link><p className="mt-2 line-clamp-2 text-xs leading-5 text-ink-soft">{[l.cpu,l.ram,l.storage,l.gpu].filter(Boolean).join(" • ")}</p><div className="mt-auto flex items-end justify-between pt-5"><div><span className="text-xl font-extrabold text-ink">${l.price}</span>{l.prev_price && <span className="mr-2 text-xs text-ink-soft/60 line-through">${l.prev_price}</span>}<p className="mt-1 text-[11px] text-ink-soft">{l.stock_quantity > 0 ? `متوفر • ${l.stock_quantity} قطع` : "غير متوفر"}</p></div><button type="button" onClick={() => toggleCompare(l.id)} className={`rounded-xl border px-3 py-2 text-[11px] font-bold ${compare.includes(l.id) ? "border-blue bg-blue/10 text-blue" : "border-line text-ink-soft hover:border-blue"}`}>{compare.includes(l.id) ? "✓ مقارنة" : "مقارنة"}</button></div></div>
    </article>)}</div>}
